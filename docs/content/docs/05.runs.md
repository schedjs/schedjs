---
title: Runs
description: The run lifecycle and RunRecord
---
...
`data` is a snapshot of the run parameters taken at dispatch: from the **schedule's**
`data` for a schedule-fired run (falling back to the task's `config.data` /
...
- **Cancelled** — timeout escalation in the docker runner (SIGTERM → SIGKILL), an
  explicit cancel, or startup recovery (see below). User cancel (`POST
  /runs/:id/cancel`) aborts the runner via `hooks.signal` — the worker is really
...
| kind | ref example | meaning |
|---|---|---|
| `s3` | `s3://my-bucket/task-reports/a/run123.md` | object key in object storage (Minio/S3) |
| `email` | message id / mailto: | email sent by the worker (receipt, not content) |
| `url` | `https://...` | external link |
| `file` | local path | local artifact (docker runner output) |
