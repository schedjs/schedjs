...
## In development — queue control, run window, bulk runs (core / storage / admin-api / cli / mcp)
...
carries the answer to "what did I miss": `{ pausedMs, skippedSchedules, deferredRuns }` (see [Self-hosting](self-hosting#pausing-the-queue)).
...
- **Run filters — RunFilter += since / until / runner.**`GET /runs?since=&until=&runner=` (ISO-8601 on the wire;
...
`queued`/`running` rows; the stated price is that a long run started *before*`since` is not listed even when it failed inside the window. Backed by
