...
no-op for unknown runs), `listRuns` (newest-first — `startedAt` DESC;
`taskName`/`status`/`runner` filters plus a `since`/`until` start-time window,
**bounds inclusive**, see [Runs → Filtering history](../runs#filtering-run-history);
`limit`/`offset`, hard cap 1000), `pruneRuns` (retention sweep:
